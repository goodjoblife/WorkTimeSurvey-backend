...
describe("Replies Test", () => {
    ...
    describe("POST /experiences/:id/replies", () => {
        ...
        it("should 200 Success if succeed", () => {
            ...
            const check_experiences_collection = req.then(() =>
                db
                    .collection("experiences")
                    .findOne({ _id: ObjectId(experience_id_string) })
                    .then(experience => {
                        assert.equal(experience.reply_count, 1);
                    })
            );

            const check_replies_collection = req.then(res =>
                db
                    .collection("replies")
    ...
    describe("GET /experiences/:id/replies", () => {
        ...
        it("should get replies, and the fields are correct", () =>
            request(app)
        ...
        it("should see liked (true/false) if autheticated", () => {
            const request_as_fake_user = request(app)
                ...
                .expect(res => {
                    ...
                    assert.isArray(res.body.replies);
                    assert.isTrue(
                        res.body.replies[0].liked,
                        "fake_user 對 reply1 按過讚"
                    );
                    assert.isTrue(
                        res.body.replies[1].liked,
                        "fake_user 對 reply2 按過讚"
                    );
                    assert.isFalse(
                        res.body.replies[2].liked,
        ...
        it("get one experiences replies , and validate return field", () =>
            request(app)
                .get(`/experiences/${experience_id_string}/replies`)
                .query({
                    limit: 1,
