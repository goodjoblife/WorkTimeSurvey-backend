...
describe('Replies 留言', () => {
    ...
    describe('PATCH /replies/:id', () => {
        ...
        it('should return 200, when autheticated user updates status',
            async () => {
                const res = await request(app).patch(`/replies/${reply_id_string}`)
                    .send({
                ...
                const reply = await db.collection('replies')
        ...
        it('should return 404, when reply does not exist',
            () =>
                request(app).patch(`/replies/a_fake_reply_id`)
                    .send({
                        access_token: 'fakeaccesstoken',
                        status: 'hidden',
                    })
        ...
        it('should return 401, when user is not authenticated',
            () =>
                request(app).patch(`/replies/${reply_id_string}`)
                    .send({
                        status: 'hidden',
                    })
                    .expect(401)
